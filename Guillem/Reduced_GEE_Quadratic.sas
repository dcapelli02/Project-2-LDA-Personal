...
proc genmod data = alzheimer_long_centered descending;
    ...
    */
    ...
    /*
    model CDRSB_CAT = LOGTIME SEX AGE_STD BMI_STD TAUPET_STD ABPET_STD
     LOGTIME * BMI_STD LOGTIME * AGE_STD LOGTIME * TAUPET_STD LOGTIME * ABPET_STD/
     */
...
proc means data=gee_preds nway noprint;
    class TIME;
    /* Compare real outcome (CDRSB_CAT) vs Model Prediction (Predicted_Prob) */
...
Our initial model failed to recognize the importance of daily functioning (ADL) 
because it assumed patients decline at a steady, straight-line pace. In reality, 
patients with poor function do not decline steadily; they accelerate into severity, 
like falling off a cliff. Once we updated the model to capture this 'curved' acceleration, 
ADL immediately became the strongest predictor because it effectively identifies who is about to crash. 
Consequently, the biomarker Tau (which was significant in the simpler model) lost its significance; 
it was merely acting as a rough placeholder until the model could correctly identify the rapid 
functional decline driven by ADL.
...
proc genmod data = alzheimer_long_centered descending;
    ...
    model CDRSB_CAT = TIME TIME*TIME SEX BMI_STD ADL TAUPET_STD 
     TIME * BMI_STD/
        dist=binomial link=logit;
