...
proc glimmix data=alzheimer_long method=RSPL empirical;
	...
	model CDRSB_CAT(descending) = TIME SEX AGE_STD BMI_STD ADL ABPET TAUPET 
...
/* Get rid of timesex, timeadl (timeab should be discussed) */
...
/* Get rid of adl and sex: again we should discuss timeab */

proc glimmix data=alzheimer_long method=RSPL empirical;
	class PATID SEX TIMECLSS;
	model CDRSB_CAT(descending) = TIME AGE_STD BMI_STD ABPET TAUPET 
     TIME * AGE_STD TIME * BMI_STD TIME * ABPET TIME * TAUPET
		/ dist=binary solution;
	random _residual_ / subject=PATID type=UN;
run;
